...
def insert_brand_ad_conversion(
    ...
    for i in range(row_count):
        query = BRAND_AD_CONVERSION_INSERT.format(
            next(counter), ws_idx, creative_idx,
...
@then(
    '간접 전환 지표가 wsIdx가 {ws_idx}인 광고주에 찜수 {like_count:d}건, 매장방문수 {ws_visit_count:d}건, 거래처 요청수 {trade_request_count:d}건, 광고 본 사용자 수 {viewer_count:d}건 발생했다.')
...
@then(
    '간접 전환 지표가 wsIdx가 {ws_idx:d}인 광고주의 {creative_idx:d}인 소재에 찜수 {like_count:d}건, 매장방문수 {ws_visit_count:d}건, 거래처 요청수 {trade_request_count:d}건, 광고 본 사용자 수 {viewer_count:d}건 발생했다.')
...
@then(
    '브랜드 광고 전환 지표가 wsIdx가 {ws_idx:d}인 광고주에 찜수 {like_count:d}건, 매장방문수 {ws_visit_count:d}건, 거래처 요청수 {trade_request_count:d}건, 광고 본 사용자 수 {viewer_count:d}건 발생했다.')
def check_brand_ad_conversion_all_by_creative_idx(
        context, ws_idx: int,
        like_count: int, ws_visit_count: int, trade_request_count: int, viewer_count: int
):
    query = BRAND_AD_CONVERSION_ALL_SELECT.format(ws_idx, ws_idx) # print(query)
    result = select_one_click_house_query(query)
    assert_equals(result[0][1], like_count)
    assert_equals(result[0][2], ws_visit_count)
    assert_equals(result[0][3], trade_request_count)
...
@then(
    '브랜드 광고 전환 지표가 wsIdx가 {ws_idx:d}인 광고주의 {creative_idx:d}인 소재에 찜수 {like_count:d}건, 매장방문수 {ws_visit_count:d}건, 거래처 요청수 {trade_request_count:d}건, 광고 본 사용자 수 {viewer_count:d}건 발생했다.')
def check_brand_ad_conversion_all_by_creative_idx(
        context, ws_idx: int, creative_idx: int,
        like_count: int, ws_visit_count: int, trade_request_count: int, viewer_count: int
):
